...
docker run --name mos_erp_environment -P --net=host -e LDAP_URL=${LDAP_URL} -e EUREKA_URL=${EUREKA_URL} -e SPRING_PROFILES_ACTIVE="docker" -d moserp/environment
docker run --name mos_erp_facility -P --net=host -e LDAP_URL=${LDAP_URL} -e EUREKA_URL=${EUREKA_URL} -e SPRING_PROFILES_ACTIVE="docker" -d moserp/facility
docker run --name mos_erp_product -P --net=host -e LDAP_URL=${LDAP_URL} -e EUREKA_URL=${EUREKA_URL} -e SPRING_PROFILES_ACTIVE="docker" -d moserp/product
docker run --name mos_erp_inventory -P --net=host -e LDAP_URL=${LDAP_URL} -e EUREKA_URL=${EUREKA_URL} -e SPRING_PROFILES_ACTIVE="docker" -d moserp/inventory
